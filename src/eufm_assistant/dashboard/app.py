...
@app.route('/strategy')
def strategy():
    print("Running Coordinator Agent to generate strategic plan...")
    settings = load_settings()
    if not settings:
        return render_template('strategy.html', error="Could not load AI assistant settings.")
    ...
    return render_template('strategy.html', plan=plan)
