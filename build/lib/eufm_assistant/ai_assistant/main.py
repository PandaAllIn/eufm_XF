...
def main():
    ...
    parser_research = subparsers.add_parser(
        "research", help="Run the research agent to find collaborators."
    )
    parser_research.add_argument(
        "query", type=str, help="The research query for the research agent."
    )

    # Coordinate command
    parser_coordinate = subparsers.add_parser(
        "coordinate", help="Run the coordinator agent for strategic tasks."
    )
    parser_coordinate.add_argument(
        "--task",
        default="wbs_status",
        help="The specific task for the coordinator agent (e.g., 'proposal_checklist').",
    )
